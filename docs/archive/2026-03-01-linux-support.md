# ARCHIVED — Implemented Design Document

> **Status:** Archived on 2026-05-31. This design was implemented as specified.
>
> **Reason:** This document accurately describes the Linux terminal adapter additions (Konsole, Hyper, Tabby, st, urxvt, VTE catch-all) that were implemented in `adapter/terminals.go` and `adapter/adapter.go`. Since the feature is complete and the code is the source of truth, this plan doc is no longer needed for active development.
>
> **Current source of truth:** `adapter/terminals.go`, `adapter/adapter.go`, and `adapter/terminals_test.go`.
