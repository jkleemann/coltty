# ARCHIVED — Stale Design Document

> **Status:** Archived on 2026-05-31. This document no longer reflects the current implementation.
>
> **Reason:** Significant implementation drift. The codebase has evolved well beyond this original design in several ways:
> - Added `coltty set` and `coltty import` commands (the latter explicitly listed as "out of scope" here)
> - Added iTerm2, Terminal.app, OSC, tmux, and color utility adapters not mentioned in this doc
> - Ghostty adapter emits OSC sequences in addition to writing config fragments
> - Shell hooks run synchronously, not in the background (`&!` was removed)
> - No `adrg/xdg` dependency; XDG paths are resolved manually
> - Project structure has grown to ~15 source files plus tests
> - Built-in schemes are hardcoded (gruvbox, nord, dracula, etc.), not generated on first run
>
> **Current source of truth:** The Go source code in the repository root and `adapter/` directory.
